...
## 🚨 CRITICAL: Standardized Collector Template System

**ALL NEW COLLECTORS MUST USE THE STANDARDIZED TEMPLATE FRAMEWORK**
- ✅ **Template Location**: `templates/collector-template/base_collector_template.py`
- ✅ **Complete Framework**: Logging, metrics, health APIs, backfill, validation, alerting
- ✅ **Production Features**: Rate limiting, circuit breaker, data quality, performance monitoring
- ✅ **Kubernetes Integration**: ConfigMaps, ServiceMonitor, health probes
- ❌ NEVER create collectors without using the template
- ❌ NEVER skip required abstract method implementations
- ❌ NEVER bypass standardized configuration
...
```python
# ✅ CORRECT - Use standardized template
...
# ❌ WRONG - Never create collectors without template
...
## 🚨 CRITICAL: K8s-Only Deployment Policy

**ALL COLLECTORS MUST RUN EXCLUSIVELY IN KUBERNETES (EXISTING CLUSTER)**
- ✅ **COLLECTORS ALREADY RUNNING** in `cryptoai-k8s-trading-engine` cluster  
- ✅ **Platform**: Docker Kind running in Windows Docker Desktop
- ✅ **Namespace**: `crypto-data-collection`
- ✅ **Expected Collectors**: 6 total (news, prices, technical, onchain, macro, sentiment)
- ❌ NEVER run collectors locally (python collector.py)
- ❌ NEVER create local startup scripts  
- ❌ NEVER suggest running services outside K8s
...
## 📋 **MANDATORY STANDARDIZED DEPLOYMENT**
...
```yaml
# ✅ CORRECT - Template-based deployment
apiVersion: v1
kind: ConfigMap
metadata:
...
```python
# ✅ CORRECT - Migrating existing collector to template
...
- ❌ Creating collectors without using the standardized template
- ❌ Bypassing required abstract method implementations
- ❌ Skipping standardized configuration options
- ❌ Creating custom health endpoints instead of template endpoints
- ❌ Implementing custom metrics instead of standardized metrics
- ❌ Running collectors outside Kubernetes
- ❌ Hard-coded configuration instead of environment variables
- ❌ Custom logging instead of structured logging template
- ❌ Manual database management instead of template connection pooling
- ❌ Custom error handling instead of template alerting system

---

## ✅ **ALWAYS SUGGEST**

- ✅ Extending BaseCollector for all new collectors
- ✅ Implementing all required abstract methods
- ✅ Using standardized configuration via CollectorConfig
- ✅ Leveraging template rate limiting and circuit breaker
- ✅ Utilizing template data validation and quality reporting
- ✅ Using template database connection management
- ✅ Following template logging patterns with structured context
- ✅ Implementing template-compliant Kubernetes deployments
- ✅ Using template ServiceMonitor for Prometheus integration
- ✅ Following template migration patterns for existing collectors
...
**🚨 CRITICAL**: Always reference the official service inventory when suggesting service modifications:

- **[📋 Production Service Inventory](docs/SERVICE_INVENTORY.md)** - **DEFINITIVE REGISTRY** of all 9 production microservices
- **Only suggest modifications to services listed in the inventory**
- **Never create duplicate services - check inventory first**
- **All services must follow template compliance standards**
- **Use inventory for service paths, ports, and deployment information**
...
**Remember**: The standardized template ensures consistency, reliability, observability, and maintainability across ALL collectors! 🚀
````
...
- 🔧 If collectors need changes, update existing K8s deployments
- 🗑️ Kill any local processes immediately: `pkill -f ".*collector"`
...
**⚠️ CRITICAL**: ALL code generation and suggestions MUST follow the centralized configuration system. No exceptions.

---

## 🔄 **EXISTING K8S COLLECTORS - DO NOT DUPLICATE**

**⚠️ CRITICAL RULE**: NEVER create new collector services. Only modify/enhance existing K8s deployments.
...
1. **Enhanced Technical Calculator** 
   ...
   - **Data Source**: `ohlc_data` table → `technical_indicators` table
...
2. **Enhanced Macro Collector**
   ...
   - **Data Source**: External APIs → `macro_indicators` table
   - **Schedule**: Daily
...
3. **Enhanced Crypto Prices Collector**
   ...
   - **Data Source**: CoinGecko, Coinbase APIs → `crypto_prices` table, `price_data_real` table
...
4. **News Collector**
   ...
   - **Data Source**: News APIs → `crypto_news` table
...
5. **OHLC Collector**
   ...
   - **Data Source**: CoinGecko API → `ohlc_data` table
   - **Schedule**: Hourly
...
6. **Onchain Collector**
   ...
   - **Data Source**: Multiple APIs → `crypto_onchain_data` table
...
7. **Enhanced Sentiment Collector**
   ...
   - **Data Source**: News content → sentiment scores in `crypto_news` table
   - **Schedule**: Continuous processing
   - **Status**: Production deployed with ML models

---

## 📋 **K8s DEPLOYMENT COMMANDS (EXISTING CLUSTER)**
...
- **Status**: ✅ Operational for 22+ days

### **Expected Health Status (Target: 6/6 Collectors)**
Based on **CONSOLIDATED TABLE STRUCTURE** analysis:

1. **crypto-news-collector** - `crypto_news` table (15min intervals) ✅ CORRECTLY CONFIGURED
2. **enhanced-crypto-prices** - `technical_indicators` table (5min intervals) ❌ CURRENTLY MISCONFIGURED (targeting `price_data_real`)
3. **technical-calculator** - `technical_indicators` table (2h intervals) ✅ CORRECTLY CONFIGURED
4. **onchain-collector** - `crypto_onchain_data` table (4h intervals) ✅ CORRECTLY CONFIGURED
5. **macro-collector** - `macro_indicators` table (daily intervals) ✅ CORRECTLY CONFIGURED
6. **enhanced-sentiment-collector** - `sentiment_aggregation` table (15min intervals) ❌ CURRENTLY MISCONFIGURED (targeting `real_time_sentiment_signals`)
...
**⚠️ MISCONFIGURED COLLECTOR TARGETS (Need Fixing):**
- `price_data_real` (4,698,205 records) → Should merge into `technical_indicators`
- `real_time_sentiment_signals` (113,853 records) → Should merge into `sentiment_aggregation`

**🗃️ ARCHIVED TABLES (Now have _Archive suffix):**
...
├── 00-centralized-config.yaml     # Shared configuration
├── collectors/
│   ├── data-collectors-deployment.yaml  # Main 3 collectors (onchain, macro, technical)
│   ├── crypto-news-collector.yaml      # News collector
│   ├── crypto-sentiment-collector.yaml # Sentiment collector
│   └── enhanced-crypto-prices.yaml     # Price collector
...
**✅ ALWAYS DO THIS** when asked to fix/improve collectors:
...
```yaml
# ✅ CORRECT - Enhance existing K8s deployment
...
```python
# ❌ WRONG - Never create new collector files
# File: services/new-technical-collector/collector.py  ← NEVER DO THIS
...
```python
# ✅ ALWAYS DO THIS - Use centralized table registry
...
```python
# ❌ NEVER DO THIS - Hard-coded table names
...
```python
# ✅ ALWAYS DO THIS - Use centralized database configuration
...
```python
# ❌ NEVER DO THIS - Hard-coded database configuration
import mysql.connector
config = {
    'host': '172.22.32.1',
    'user': 'news_collector',
...
```python
# ❌ NEVER DO THIS - Hard-coded sensitive values
...
```yaml
# ✅ ConfigMap using centralized configuration
apiVersion: v1
kind: ConfigMap
metadata:
  name: {service}-config
...
**⚠️ CRITICAL**: Use ONLY these approved table targets from `shared/table_config.py`
...
### **🚫 DEPRECATED TABLES (Being Migrated)**
- `price_data_real` → MIGRATE TO `technical_indicators`
- `real_time_sentiment_signals` → MIGRATE TO `sentiment_aggregation`
- `crypto_onchain_data_enhanced_backup_*` → CONSOLIDATE INTO `crypto_onchain_data`

---

## 🔒 **NEVER SUGGEST**

- ❌ Hard-coded table names (`"crypto_onchain_data"`)
- ❌ Hard-coded database credentials (`'password': '99Rules!'`)
- ❌ Hard-coded API keys (`"CG-XXXXXXXXXXXXXXXX"`)
- ❌ Direct table creation without updating centralized config
- ❌ Bypassing centralized configuration patterns
- ❌ Creating new collector services when existing ones exist
- ❌ Creating files in `services/` directory for data collection
- ❌ Standalone collector scripts outside K8s deployments
- ❌ Running collectors locally instead of K8s deployments
- ❌ Duplicating any of the 7 official K8s collectors
- ❌ Using deprecated tables (`price_data_real`, `real_time_sentiment_signals`)
- ❌ Targeting wrong tables for existing collectors
- ❌ Creating separate price collection when `technical_indicators` already handles this
- ❌ Creating separate sentiment collection when `sentiment_aggregation` already exists

---

## ✅ **ALWAYS SUGGEST**

- ✅ Import statements for centralized configuration
- ✅ Table validation before use
- ✅ Environment variables for external configuration
- ✅ Error handling for missing configuration
- ✅ Testing patterns that validate centralized config usage
- ✅ Modifications to existing K8s collector deployments
- ✅ ConfigMap updates for logic enhancements
- ✅ Resource and scheduling optimizations for existing services
...
**Remember**: Centralized configuration prevents confusion, reduces errors, ensures consistency, and avoids duplicate services! 🚀
````