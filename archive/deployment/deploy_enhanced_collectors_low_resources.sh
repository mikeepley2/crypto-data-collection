...
echo "🔧 Updating Enhanced Collectors with Lower Resource Limits..."

# Delete existing enhanced deployments and recreate with lower limits
kubectl delete deployment enhanced-crypto-news-updated -n crypto-data-collection --ignore-not-found
kubectl delete deployment enhanced-macro-indicators -n crypto-data-collection --ignore-not-found  
kubectl delete deployment enhanced-onchain-data -n crypto-data-collection --ignore-not-found
kubectl delete deployment enhanced-technical-calculator -n crypto-data-collection --ignore-not-found
kubectl delete deployment enhanced-crypto-prices-updated -n crypto-data-collection --ignore-not-found

echo "📦 Deploying enhanced crypto news collector with low resources..."
kubectl apply -f - << 'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
...
echo "📦 Deploying enhanced technical calculator with low resources..."
kubectl apply -f - << 'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
    ...
    spec:
      containers:
        - name: enhanced-technical-calculator
          ...
          args:
            - |
              pip install mysql-connector-python requests numpy pandas talib
                  ...
                  def calculate_technical_indicators(self, symbol: str) -> Optional[Dict]:
                      try:
                          ...
                          return {
                              ...
                              "macd_histogram": macd_histogram.iloc[-1] if not pd.isna(macd_histogram.iloc[-1]) else None
                          }
                          ...
                          with self.get_db_connection() as conn:
                              ...
                              cursor.execute("""
                                  INSERT INTO technical_indicators (
                                      ...
                                      rsi, macd, macd_signal, macd_histogram, calculated_at
                                  ) VALUES (
...
echo "✅ Enhanced collectors deployed successfully!"
