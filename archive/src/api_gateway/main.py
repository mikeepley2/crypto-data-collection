...
async def health_check():
    ...
    # Overall status
    status = (
        "healthy" if all(c == "healthy" for c in components.values()) else "degraded"
    )
...
async def get_latest_crypto_news(
    limit: int = Query(20, le=100), access_level: str = Depends(verify_api_key)
):
...
async def get_technical_indicators(
    symbol: str, access_level: str = Depends(verify_api_key)
):
    ...
    async with mysql_pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            ...
            if not result:
                raise HTTPException(
                    status_code=404,
                    detail=f"Technical indicators not found for {symbol}",
                )
...
async def get_current_ml_features(
    ...
    async with mysql_pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cursor:
            ...
            if not result:
                raise HTTPException(
                    status_code=404, detail=f"ML features not found for {symbol}"
                )
