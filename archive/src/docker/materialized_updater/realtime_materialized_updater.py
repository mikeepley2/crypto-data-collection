...
# Import enhanced ML calculations and database config
sys.path.append('/mnt/e/git/crypto-data-collection')
from enhanced_ml_calculations import AdvancedMLCalculator
...
class RealTimeMaterializedTableUpdater:
        ...
        try:
            ...
            if not existing:
                ...
                INSERT INTO ml_features_materialized (
                    ...
                    data_quality_score,
                    risk_parity_signal, momentum_factor, carry_trade_signal,
                    flight_to_quality, macro_surprise_index, sentiment_regime,
                    liquidity_stress, volatility_term_structure, regime_transition_prob,
                    systemic_risk_indicator, ml_confidence_score
                ) VALUES (
                    ...
                    %(risk_parity_signal)s, %(momentum_factor)s, %(carry_trade_signal)s,
                    %(flight_to_quality)s, %(macro_surprise_index)s, %(sentiment_regime)s,
                    %(liquidity_stress)s, %(volatility_term_structure)s, %(regime_transition_prob)s,
                    %(systemic_risk_indicator)s, %(ml_confidence_score)s
                )
                """
    ...
    def get_new_price_data(self, symbol, since_timestamp, end_date):
        ...
        try:
            ...
            query = """
            SELECT 
                symbol, timestamp_iso as timestamp_iso, price as current_price, 
                volume as volume_usd_24h, NULL as volume_qty_24h, 
                NULL as hourly_volume_usd, NULL as hourly_volume_qty,
                market_cap as market_cap, price_change_24h, 
            ...
            if max_time:
                query += " AND timestamp_iso <= %s"
                params.append(max_time)
            query += " AND price IS NOT NULL ORDER BY timestamp_iso ASC"
    ...
    def process_symbol_updates(self, symbol):
        ...
        for hour_key, records in hourly_records.items():
            ...
            try:
                ...
                # Advanced ML Calculations (columns 201-211)
                t5 = _time.time()
                try:
                    ml_indicators = self.ml_calculator.calculate_advanced_ml_indicators(
                        symbol, timestamp_iso, record
                    )
                    logger.info(
                        f"⏱️ Advanced ML calculations: {(_time.time()-t5):.3f}s for {symbol} {timestamp_iso}"
                    )
                    
                    # Check if ML indicators need updating
                    ml_fields = [
                        'risk_parity_signal', 'momentum_factor', 'carry_trade_signal',
                        'flight_to_quality', 'macro_surprise_index', 'sentiment_regime',
                        'liquidity_stress', 'volatility_term_structure', 'regime_transition_prob',
                    ...
                    if (not existing or any((existing or {}).get(f) is None for f in ml_fields)):
                        record.update(ml_indicators)
                        need_update = True
                        logger.debug(f"Added advanced ML indicators for {symbol} {timestamp_iso}")
                    
                except Exception as e:
                    logger.error(f"Error calculating advanced ML indicators for {symbol}: {e}")
                    # Add default values to prevent SQL errors
                    record.update(self.ml_calculator.get_default_ml_indicators())
    ...
    def __init__(self):
        ...
        # Initialize ML calculator with centralized DB config
        self.ml_calculator = AdvancedMLCalculator(get_db_config())
