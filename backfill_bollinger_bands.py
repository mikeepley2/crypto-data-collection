...
def calculate_bollinger_bands():
    ...
    try:
        ...
        cursor.execute("SELECT DISTINCT symbol FROM technical_indicators ORDER BY symbol")
        ...
        for idx, symbol in enumerate(symbols, 1):
            ...
            try:
                ...
                for record_id, sym, timestamp_iso, sma_20 in records:
                    ...
                    # Get last 20 prices before this timestamp
                    cursor.execute("""
                    ...
                    if len(prices) >= 2:  # Need at least 2 prices for std dev
                        prices = sorted(prices, reverse=False)  # Get in chronological order
        ...
        print(f"Success rate: {(total_updated/total_processed*100):.1f}%" if total_processed > 0 else 0)
        print()
        ...
        print(f"Bollinger Bands now populated: {verification[0]:,} / {verification[1]:,} ({bb_pct:.1f}%)")
