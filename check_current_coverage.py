#!/usr/bin/env python3
"""
Check current column coverage using correct environment variables
"""
...
def check_coverage():
    try:
        ...
        cursor.execute(
            "SELECT COUNT(close_price) FROM ml_features_materialized WHERE DATE(timestamp_iso) = CURDATE()"
        )
        close_price = cursor.fetchone()[0]
        print(f"Close Price: {close_price:,} ({close_price/total*100:.1f}%)")
        ...
        print(f"Macro: {macro:,} ({macro/total*100:.1f}%)")
