...
def validate_onchain_data():
    ...
    try:
        ...
        print(f"Records with ALL 4 metrics: {complete[1]:,} / {complete[0]:,} ({complete_pct:.2f}%)")
        print()
        ...
        if stats[0] is not None:
            print(f"Active Addr:   MIN={stats[0]:>12.0f}, MAX={stats[1]:>12.0f}, AVG={stats[2]:>12.0f}")
            print(f"Tx Count:      MIN={stats[3]:>12.0f}, MAX={stats[4]:>12.0f}, AVG={stats[5]:>12.0f}")
            print(f"Exchange Flow: MIN={stats[6]:>12.4f}, MAX={stats[7]:>12.4f}, AVG={stats[8]:>12.4f}")
            print(f"Volatility:    MIN={stats[9]:>12.4f}, MAX={stats[10]:>12.4f}, AVG={stats[11]:>12.4f}")
        else:
        ...
        print(f"{'Symbol':<8} | {'Total':>8} | {'Addr%':>7} | {'Tx%':>7} | {'Flow%':>7} | {'Latest':20}")
        print("-" * 75)
        ...
        for sym in symbols:
            ...
            print(f"{sym[0]:<8} | {sym[1]:>8,} | {addr_pct:>6.1f}% | {tx_pct:>6.1f}% | {flow_pct:>6.1f}% | {latest:20}")
        ...
        if all_pass:
            ...
            print(f"  - Active Addresses:        {(metrics['Active Addresses 24h']/total*100):.2f}%")
            print(f"  - Transaction Count:       {(metrics['Transaction Count 24h']/total*100):.2f}%")
            print(f"  - Exchange Net Flow:       {(metrics['Exchange Net Flow 24h']/total*100):.2f}%")
            print(f"  - Price Volatility:        {(metrics['Price Volatility 7d']/total*100):.2f}%")
            ...
            for issue in issues:
                print(f"  - {issue}")
            print()
            print("Note: Onchain data has lower coverage due to API limitations (expected)")
