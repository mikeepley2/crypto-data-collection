...
def monitor_collection_health():
    ...
    try:
        ...
        # Check recent data collection activity with correct column names
        checks = [
            {
        ...
        for check in checks:
            ...
            if latest:
                ...
                print(f"   Recent: {recent_1h} (1h), {recent_24h} (24h)")
        ...
        for check in news_checks:
            news_cursor.execute(check['query'])
...
def update_monitoring_status(health_status, sentiment_status):
    ...
    try:
        ...
        # Update collection monitoring records
        for service_name, status_info in health_status.items():
            service_key = service_name.lower().replace(' ', '_').replace('_collection', '')
            ...
            """, (
                status_info['status'][:10],  # Truncate status
                status_info['latest'],
                status_info['recent_24h'],
                f"Age: {status_info['age_hours']:.1f}h" if status_info['age_hours'] != float('inf') else "No data",
                f"%{service_key}%"
            ))
        ...
        print(f"✅ Updated monitoring status for {len(health_status)} services")
        print(f"✅ Created/updated {alerts_created} alerts")
...
def main_monitoring_cycle():
    ...
    try:
        ...
        alerts_created = update_monitoring_status(health_status, sentiment_status)
...
def continuous_monitoring(check_interval=600):  # 10 minutes
...
if __name__ == "__main__":
    ...
    print("✅ Initial monitoring cycle completed")
    print("📊 Monitoring tables updated")
