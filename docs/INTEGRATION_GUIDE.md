...
## � **MANDATORY CENTRALIZED CONFIGURATION**

**⚠️ CRITICAL**: All integrations MUST use the centralized configuration system.

### **📋 Configuration Standards for Integrations**
- **[Centralized Config Instructions](../CENTRALIZED_CONFIG_INSTRUCTIONS.md)** - **MANDATORY** reading
- **[Table Configuration](../shared/table_config.py)** - Master table registry for all integrations
- **[Database Configuration](../shared/database_config.py)** - Centralized DB config for all connections

### **🔧 Integration Requirements**
All systems integrating with the data collection system must:
- ✅ Use centralized table registry for all database operations
- ✅ Reference environment variables for API configuration
- ✅ Validate table names before use
- ✅ Use centralized database connection methods
- ❌ Never hard-code table names, database credentials, or API endpoints

```python
# ✅ CORRECT - Integration using centralized config
from shared.table_config import CRYPTO_TABLES, TECHNICAL_TABLES
from shared.database_config import get_db_connection

# Access master tables through centralized registry
onchain_data = CRYPTO_TABLES["ONCHAIN_DATA"]     # "crypto_onchain_data"
ml_features = ML_TABLES["FEATURES"]              # "ml_features_materialized"
...
**⚠️ MANDATORY**: All database connections MUST use centralized configuration.

#### **Centralized Database Configuration Pattern**

```python
# ✅ CORRECT - All services must use this pattern
...
```yaml
# ✅ CORRECT - Kubernetes ConfigMap using centralized configuration
apiVersion: v1
kind: ConfigMap
metadata:
...
data:
  # Database connection parameters (used by centralized config)
  ...
  # Centralized configuration indicators
  USE_CENTRALIZED_CONFIG: "true"
  TABLE_CONFIG_PATH: "/app/shared/table_config.py"
  DATABASE_CONFIG_PATH: "/app/shared/database_config.py"
```

```yaml
# ❌ DEPRECATED - Old hard-coded configuration (DO NOT USE)
apiVersion: v1
kind: ConfigMap
metadata:
  name: old-database-config
data:
  MYSQL_HOST: "host.docker.internal"
  MYSQL_DATABASE_PRICES: "crypto_prices"
  # Hard-coded table names (DEPRECATED)
  ONCHAIN_TABLE: "crypto_onchain_data"           # Use CRYPTO_TABLES["ONCHAIN_DATA"] instead
  ASSETS_TABLE: "crypto_assets"                  # Use CRYPTO_TABLES["ASSETS"] instead
```
...
cursor.execute(f"""
    ...
    WHERE symbol = %s AND timestamp = (
        SELECT MAX(timestamp) FROM {ml_table} WHERE symbol = %s
    )
""", (symbol, symbol))

# Query onchain data for additional signals
cursor.execute(f"""
...
```python
# ❌ WRONG - Hard-coded database configuration (DO NOT USE)
