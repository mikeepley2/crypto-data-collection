...
class EnhancedMacroCollector:
    ...
    def __init__(self):
        ...
        logger.info(f"📊 Tracking {len([i for i in self.indicators if i.active])} active indicators")
        logger.info(f"🔑 Using FRED API: {self.fred_api_key[:8]}...")
    ...
    def fetch_indicator_data(self, indicator: MacroIndicator, start_date: date = None, end_date: date = None) -> List[Dict]:
        ...
        try:
            ...
            logger.debug(f"📡 Fetching {indicator.name} from FRED API")
